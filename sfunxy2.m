function [sys, x0, str, ts, simStateCompliance] = sfunxy2(t,x,u,flag,ax,varargin)
...
ud.XYLine   = new_line;
ud.XYXlabel = xlabel('X Axis (km)');
ud.XYYlabel = ylabel('Z Axis (km)');
ud.XYTitle  = get(ud.XYAxes,'Title');
ud.XData    = [];
ud.YData    = [];

set(ud.XYTitle,'String','2D Plot, Lintasan Orbit');
...
if isa(lineobj,'hg2sample.ScopeLineAnimator')
