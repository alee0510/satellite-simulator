...
%
   handle.axes(1)=axes;
   axis(Config.axes);
   set(handle.axes(1),'visible','off','xtick',[],'ytick',[],'ztick',[],'box','on', ...
           'dataaspectratio',[1 1 1], ...
           'projection','pers', ...
           'units','normal', ...
           'position',[0.3 0.1 0.75 0.75], ...
           'Color',[.8 .8 .8], ...
           'drawmode','fast', ...
           'clipping','off');
   set(gcf,'color',[0 0 0])
...
%   
% Figure
Re = 3000; %earth's radius, km

% load earth's image, .jpg
image_file = 'Earth_1024x512.jpg';
cdata = imread(image_file);

%base point on earth
[x,y,z] = ellipsoid(0, 0, 0, Re, Re, Re, 30);

%plot the sufface of ellipsoid, map-pojection
globe = surface(x, y, -z, 'FaceColor', 'none', 'EdgeColor', 0.5*[1 1 1]);

%applying properties of the surface setting up the image_file
set(globe, 'FaceColor', 'texturemap', 'CData', cdata, 'FaceAlpha', 0.99, 'EdgeColor', 'none');
grid on;
axis equal;
...
set(handle.target2,'linestyle','-','color',[1 0 0],'linewidth',1);
...
text(10000,100,100,'X','color','b');
text(100,10000,100,'Y','color','b');
text(100,100,10000,'Z','color','b');
%
...
%
   ...
   set(handle.craft,'erasemode','nor','edgecolor',[0.6 0 0.4],'clipping','off');
%
...
%=============================================================================
%
function sys=mdlGetTimeOfNextVarHit(t,~,~,Config)
